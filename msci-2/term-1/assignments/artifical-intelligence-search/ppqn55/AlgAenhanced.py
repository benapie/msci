...
def h(state):
    ...
    Calculates the length of the greedy solution from the current end node on the partial tour to a goal node.
    :param state: the current state.
    :return: the heuristic as described.
    """
    greedy_solution = [state[len(state) - 1]]
    h = 0
    # given a state, greedy solution to the goal state
    unvisited_city_list = get_unvisited_city_list(state)
    greedy_solution_length = len(unvisited_city_list) + 1
    while len(greedy_solution) < greedy_solution_length:
        minimum_distance = distance_matrix[greedy_solution[len(greedy_solution) - 1]][unvisited_city_list[0]]
        closest_node = unvisited_city_list[0]
        for i in range(1, len(unvisited_city_list)):
            temp_distance = distance_matrix[greedy_solution[len(greedy_solution) - 1]][unvisited_city_list[i]]
            if temp_distance < minimum_distance:
                minimum_distance = temp_distance
                closest_node = unvisited_city_list[i]
        greedy_solution.append(closest_node)
        h += minimum_distance
        unvisited_city_list.remove(closest_node)
    return h

def get_unvisited_city_list(state):
    """
    :param state: the current state.
    :return: list of unvisited cities.
    """
    city_list = [i for i in range(0, num_cities)]
    for i in state:
        city_list.remove(i)
    return city_list


def add_to_fringe(f, n):
    """
    Adds a node to the fringe priority queue.
    :param f: fringe.
    :param n: node to add.
    """
...
def a_star():
    """
    Performs the A* algorithm on the distance matrix.
    :return: a tuple of (id, tour, tour_length)
    """

    # Initialise
    new_id = 0
    fringe = [(new_id, [0], 0, h([0]))]
    start = time.process_time()

    # Start the algorithm
    while len(fringe) > 0:
        # Get the lowest heuristic node and get the list of children.
        instance = fringe.pop(0)
        unvisited_city_list = get_unvisited_city_list(instance[1])

        # If the algorithm takes just under 2 minutes and hasn't terminated, calculate the greedy solution
        # from where the A* search left off.
        if time.process_time() - start > 110:
            start = time.process_time()
            greedy_solution = [instance[1][len(instance[1]) - 1]]
            unvisited_city_list = get_unvisited_city_list(instance[1])
            greedy_solution_length = len(unvisited_city_list) + 1
            total = 0
            while len(greedy_solution) < greedy_solution_length:
                ...
                unvisited_city_list.remove(closest_node)
            return (instance[0],
                    instance[1] + greedy_solution[1:],
                    instance[2] + total + distance_matrix[greedy_solution[len(greedy_solution) - 1]][0])

        # If the current selected node is a goal node, return it.
        if len(instance[1]) == num_cities:
            return (new_id,
                    instance[1],
                    instance[2] + distance_matrix[instance[1][len(instance[1]) - 1]][instance[1][0]])
        for city in unvisited_city_list:
            # Add all the children nodes to the fringe.
            new_id += 1
            distance_from_root = instance[2] + distance_matrix[instance[1][len(instance[1]) - 1]][city]
            add_to_fringe(fringe,
                          (new_id,
                           instance[1] + [city],
                           distance_from_root,
                           distance_from_root + h(instance[1] + [city])))
