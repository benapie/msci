...
def get_tour_length(tour):
    """
    :param tour: a tour.
    :return: length of a tour.
    """
    length = 0
    for i in range(0, len(tour) - 1):
        length += distance_matrix[tour[i]][tour[i + 1]]
    length += distance_matrix[tour[len(tour) - 1]][tour[0]]
    return length


def sim_ann():
    """
    Runs the simulated annealing algorithm on distance matrix.
    :return: a 2-tuple (tour, tour_length).
    """
    # first start with an arbitrary tour
    current = [i for i in range(0, num_cities)]
    T = 100
    beta = 0.999993
    epsilon = 1
    best = get_tour_length(current)
    while True:
        T = T * beta
        if T <= epsilon:
            return current, get_tour_length(current)
        else:
            a = random.randint(0, num_cities - 1)
            b = random.randint(0, num_cities - 1)
            succ = list(current)
            succ[a], succ[b] = succ[b], succ[a]
            diff = get_tour_length(succ) - get_tour_length(current)
            if diff <= 0:
                current = succ
            else:
                if random.random() < e ** (-diff / T):
                    current = succ
        if get_tour_length(current) < best:
            best = get_tour_length(current)
